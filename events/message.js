...
module.exports = async message => {
  ...
  if (cmd) {
    if (cmd.conf.enabled === false) {
      if (!ayarlar.sahip.includes(message.author.id)) {
        const embed = new Discord.MessageEmbed()
                    .setDescription(`**${cmd.help.name}** Adlı Komut Herkesin Kullanımına Kapalı!`)
                    .setColor("RED")
    ...
    let karaliste1 = db.fetch(`karaliste1_${message.author.id}`);
  if (karaliste1)
    return message.channel.send(
      new Discord.MessageEmbed()
.setDescription(`${message.author} Botun komutlarını kullanma erişimin kapatılmış!`)
